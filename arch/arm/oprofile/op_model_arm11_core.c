...
{
	u32 val = -(u32)
		counter_config[CPU_COUNTER(smp_processor_id(), cnt)].count;
	switch (cnt) {
	case CCNT:
...
int arm11_setup_pmu(void)
{
	...
	if (arm11_read_pmnc() & PMCR_E) {
		printk(KERN_ERR "oprofile: CPU%u PMU still enabled when
			setup new event counter.\n", smp_processor_id());
		return -EBUSY;
	}
	...
	for (pmnc = 0, cnt = PMN0; cnt <= CCNT; cnt++) {
		...
		if (!counter_config[
			CPU_COUNTER(smp_processor_id(), cnt)].enabled)
			continue;

		event = counter_config
			[CPU_COUNTER(smp_processor_id(), cnt)].event & 255;
...
static irqreturn_t arm11_pmu_interrupt(int irq, void *arg)
{
	...
	for (cnt = PMN0; cnt <= CCNT; cnt++) {
		if ((pmnc & (PMCR_OFL_PMN0 << cnt)) &&
			(pmnc & (PMCR_IEN_PMN0 << cnt))) {
			arm11_reset_counter(cnt);
			oprofile_add_sample(regs,
...
int arm11_request_interrupts(int *irqs, int nr)
{
	...
	for (i = 0; i < nr; i++) {
		ret = request_irq(irqs[i],
			arm11_pmu_interrupt, IRQF_DISABLED, "CP15 PMU", NULL);
		if (ret != 0) {
