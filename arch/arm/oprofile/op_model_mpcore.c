 ...
 * (not the userspace-visible counter number)
 */
static inline void scu_reset_counter(struct eventmonitor __iomem *emc,
					unsigned int n)
{
	writel(-(u32)counter_config[SCU_COUNTER(n)].count, &emc->MC[n]);
}

static inline void scu_set_event(struct eventmonitor __iomem *emc,
					unsigned int n, u32 event)
{
	event &= 0xff;
...
static void scu_stop(void)
{
	...
	/* Don't disable all 8 counters (with the E bit) as they may be in
	   use */
	temp = readl(&emc->PMCR);
	for (i = 0; i < NUM_SCU_COUNTERS; i++) {
		if (scu_em_used & (1 << i))
...
static int em_start(void)
{
	...
	if (ret == 0) {
		...
		ret = scu_start();
		if (ret)
			arm11_release_interrupts(arm11_irqs,
