...
#include "../include/asm/div64.h"	/* For division */
 ...
 */
//#include "../mach-omap2/resource34xx.h"	/* For set_opp() */
					...
					unsigned long speedup_interval)
{
	...
	if(stat->base_initialized == 0){
		...
		stat->cpu_op_index = DS_CPU_OP_INDEX_MIN;
...
#if 0	// Composed interval strategy.
				/* Calculate current interval window's utilization */
				if(stat->time_usec_interval_in_window == 0){
					stat->utilization_int_ulong = 1;
					stat->utilization_fra_fp12 = 0;
				}
				else{
					if(ds_fpdiv(stat->time_usec_work_fse_in_window,
								0,
								stat->time_usec_interval_in_window,
								0,
...
#endif
#endif	// Composed interval strategy.
...
int ds_initialize_dvs_suite(int cpu_mode){
	...
	ds_gpschedvs_status.aidvs_l3_status.current_interval_window_size =
		DS_GPSCHEDVS_L3_INTERVAL_WINDOW_SIZE;
	ds_gpschedvs_status.aidvs_l3_status.current_speedup_threshold =
		DS_GPSCHEDVS_L3_SPEEDUP_THRESHOLD;
	ds_gpschedvs_status.aidvs_l3_status.current_speedup_interval =
		DS_GPSCHEDVS_L3_SPEEDUP_INTERVAL;
	...
	ds_gpschedvs_status.aidvs_l2_status.current_interval_window_size =
		DS_GPSCHEDVS_L2_INTERVAL_WINDOW_SIZE;
	ds_gpschedvs_status.aidvs_l2_status.current_speedup_threshold =
		DS_GPSCHEDVS_L2_SPEEDUP_THRESHOLD;
	ds_gpschedvs_status.aidvs_l2_status.current_speedup_interval =
		DS_GPSCHEDVS_L2_SPEEDUP_INTERVAL;
	...
	ds_gpschedvs_status.aidvs_l1_status.current_interval_window_size =
		DS_GPSCHEDVS_L1_INTERVAL_WINDOW_SIZE;
	ds_gpschedvs_status.aidvs_l1_status.current_speedup_threshold =
		DS_GPSCHEDVS_L1_SPEEDUP_THRESHOLD;
	ds_gpschedvs_status.aidvs_l1_status.current_speedup_interval =
		DS_GPSCHEDVS_L1_SPEEDUP_INTERVAL;
	...
	ds_gpschedvs_status.aidvs_l0_status.current_interval_window_size =
		DS_GPSCHEDVS_L0_INTERVAL_WINDOW_SIZE;
	ds_gpschedvs_status.aidvs_l0_status.current_speedup_threshold =
		DS_GPSCHEDVS_L0_SPEEDUP_THRESHOLD;
	ds_gpschedvs_status.aidvs_l0_status.current_speedup_interval =
		DS_GPSCHEDVS_L0_SPEEDUP_INTERVAL;
...
int ds_update_priority_normal(struct task_struct *p){
	...
	switch(ds_status.type[p->pid]){
		case DS_HRT_TASK:
			set_user_nice(p, DS_HRT_NICE);
			break;
		case DS_SRT_UI_SERVER_TASK:
			set_user_nice(p, DS_DBSRT_NICE);
			break;
		case DS_SRT_IA_TASK:
			set_user_nice(p, DS_DBSRT_NICE);
			break;
		...
		case DS_SRT_DAEMON_TASK:
			set_user_nice(p, DS_RBSRT_NICE);
			break;
		case DS_NRT_TASK:
			set_user_nice(p, DS_NRT_NICE);
			break;
		case 0:
			set_user_nice(p, DS_NRT_NICE);
			break;
	...
	if(ds_parameter.entry_type == DS_ENTRY_SWITCH_TO){
		...
		if(ds_parameter.next_p != 0){
			...
			if(ds_parameter.next_p->pid != 0){
				...
				switch(*(ds_parameter.next_p->comm+0)){
					case 'a':
#if 0	// audio out. Audio related [daemon process]. HRT.
						if(
							*(ds_parameter.next_p->comm+1) == 'u' &&
							*(ds_parameter.next_p->comm+2) == 'd' &&
							*(ds_parameter.next_p->comm+3) == 'i' &&
							*(ds_parameter.next_p->comm+4) == 'o' &&
							*(ds_parameter.next_p->comm+5) == ' ' &&
							*(ds_parameter.next_p->comm+6) == 'o' &&
							*(ds_parameter.next_p->comm+7) == 'u' &&
							*(ds_parameter.next_p->comm+8) == 't'
						)
						{
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'd' &&
							*(ds_parameter.next_p->comm+2) == 'b' &&
							*(ds_parameter.next_p->comm+3) == 'd'
						)
						{
							new_type = DS_SRT_UI_SERVER_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						break;
					case 'b':
#if 0	// binder. [kernel thread]. < HRT.
						if(
							*(ds_parameter.next_p->comm+1) == 'i' &&
							*(ds_parameter.next_p->comm+2) == 'n' &&
							*(ds_parameter.next_p->comm+3) == 'd' &&
							*(ds_parameter.next_p->comm+4) == 'e' &&
							*(ds_parameter.next_p->comm+5) == 'r'
						)
						{
							new_type = DS_SRT_UI_SERVER_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							*(ds_parameter.next_p->comm+1) == 't' &&
							*(ds_parameter.next_p->comm+2) == 'l' &&
							*(ds_parameter.next_p->comm+3) == 'd'
						)
						{
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'l' &&
							*(ds_parameter.next_p->comm+2) == 'u' &&
							*(ds_parameter.next_p->comm+3) == 'e' &&
							*(ds_parameter.next_p->comm+4) == 't' &&
							*(ds_parameter.next_p->comm+5) == 'o' &&
							*(ds_parameter.next_p->comm+6) == 'o' &&
							*(ds_parameter.next_p->comm+7) == 't' &&
							*(ds_parameter.next_p->comm+8) == 'h' &&
							*(ds_parameter.next_p->comm+9) == 'd'
						)
						{
							new_type = DS_SRT_UI_SERVER_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
					...
					case 'c':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'o' && 
							*(ds_parameter.next_p->comm+2) == 'm' && 
							*(ds_parameter.next_p->comm+3) == '.' && 
							*(ds_parameter.next_p->comm+4) == 'l' && 
							*(ds_parameter.next_p->comm+5) == 'g' && 
							*(ds_parameter.next_p->comm+6) == 'e' && 
							*(ds_parameter.next_p->comm+7) == '.' && 
							*(ds_parameter.next_p->comm+8) == 'o' && 
							*(ds_parameter.next_p->comm+9) == 's' && 
							*(ds_parameter.next_p->comm+10) == 'p'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							...
							*(ds_parameter.next_p->comm+6) == 'e' && 
							*(ds_parameter.next_p->comm+7) == '.' && 
							*(ds_parameter.next_p->comm+8) == 'm' && 
							*(ds_parameter.next_p->comm+9) == 'e' && 
							*(ds_parameter.next_p->comm+10) == 'd' &&
							*(ds_parameter.next_p->comm+11) == 'i' &&
							*(ds_parameter.next_p->comm+12) == 'a'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							...
							*(ds_parameter.next_p->comm+2) == 'm' &&
							*(ds_parameter.next_p->comm+3) == '.' &&
							*(ds_parameter.next_p->comm+4) == 'a' &&
							*(ds_parameter.next_p->comm+5) == 'n' &&
							*(ds_parameter.next_p->comm+6) == 'd' &&
							*(ds_parameter.next_p->comm+7) == 'r' &&
							*(ds_parameter.next_p->comm+8) == 'o' &&
							*(ds_parameter.next_p->comm+9) == 'i' &&
							*(ds_parameter.next_p->comm+10) == 'd' &&
							*(ds_parameter.next_p->comm+11) == '.' &&
							*(ds_parameter.next_p->comm+12) == 'm' &&
							*(ds_parameter.next_p->comm+13) == 'm' &&
							*(ds_parameter.next_p->comm+14) == 's'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							...
							*(ds_parameter.next_p->comm+2) == 'm' &&
							*(ds_parameter.next_p->comm+3) == '.' &&
							*(ds_parameter.next_p->comm+4) == 'b' &&
							*(ds_parameter.next_p->comm+5) == 't' &&
							*(ds_parameter.next_p->comm+6) == '.' &&
							*(ds_parameter.next_p->comm+7) == 'a' &&
							*(ds_parameter.next_p->comm+8) == 'p' &&
							*(ds_parameter.next_p->comm+9) == 'p' &&
							*(ds_parameter.next_p->comm+10) == '.' &&
							*(ds_parameter.next_p->comm+11) == 'p' &&
							*(ds_parameter.next_p->comm+12) == 'b' &&
							*(ds_parameter.next_p->comm+13) == 'a' &&
							*(ds_parameter.next_p->comm+14) == 'p'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
					...
					case 'd':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'b' && 
							*(ds_parameter.next_p->comm+2) == 'u' && 
							*(ds_parameter.next_p->comm+3) == 's' && 
							*(ds_parameter.next_p->comm+4) == '-' && 
							*(ds_parameter.next_p->comm+5) == 'd' && 
							*(ds_parameter.next_p->comm+6) == 'a' && 
							*(ds_parameter.next_p->comm+7) == 'e' && 
							*(ds_parameter.next_p->comm+8) == 'm' && 
							*(ds_parameter.next_p->comm+9) == 'o' && 
							*(ds_parameter.next_p->comm+10) == 'n'
						)
						{
							new_type = DS_SRT_UI_SERVER_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'r' && 
							*(ds_parameter.next_p->comm+2) == 'o' && 
							*(ds_parameter.next_p->comm+3) == 'i' && 
							*(ds_parameter.next_p->comm+4) == 'd' && 
							*(ds_parameter.next_p->comm+5) == '.' && 
							*(ds_parameter.next_p->comm+6) == 'l' && 
							*(ds_parameter.next_p->comm+7) == 'g' && 
							*(ds_parameter.next_p->comm+8) == 'e' && 
							*(ds_parameter.next_p->comm+9) == 'c' && 
							*(ds_parameter.next_p->comm+10) == 'a' &&
							*(ds_parameter.next_p->comm+11) == 'm' &&
							*(ds_parameter.next_p->comm+12) == 'e' &&
							*(ds_parameter.next_p->comm+13) == 'r' &&
							*(ds_parameter.next_p->comm+14) == 'a'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							*(ds_parameter.next_p->comm+1) == '.' && 
							*(ds_parameter.next_p->comm+2) == 'p' && 
							*(ds_parameter.next_p->comm+3) == 'r' && 
							*(ds_parameter.next_p->comm+4) == 'o' && 
							*(ds_parameter.next_p->comm+5) == 'c' && 
							*(ds_parameter.next_p->comm+6) == 'e' && 
							*(ds_parameter.next_p->comm+7) == 's' && 
							*(ds_parameter.next_p->comm+8) == 's' && 
							*(ds_parameter.next_p->comm+9) == '.' && 
							*(ds_parameter.next_p->comm+10) == 'm' &&
							*(ds_parameter.next_p->comm+11) == 'e' &&
							*(ds_parameter.next_p->comm+12) == 'd' &&
							*(ds_parameter.next_p->comm+13) == 'i' &&
							*(ds_parameter.next_p->comm+14) == 'a'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
						...
						if(
							...
							*(ds_parameter.next_p->comm+4) == 'd' && 
							*(ds_parameter.next_p->comm+5) == '.' && 
							*(ds_parameter.next_p->comm+6) == 'b' && 
							*(ds_parameter.next_p->comm+7) == 'l' && 
							*(ds_parameter.next_p->comm+8) == 'u' && 
							*(ds_parameter.next_p->comm+9) == 'e' && 
							*(ds_parameter.next_p->comm+10) == 't' &&
							*(ds_parameter.next_p->comm+11) == 'o' &&
							*(ds_parameter.next_p->comm+12) == 'o' &&
							*(ds_parameter.next_p->comm+13) == 't' &&
							*(ds_parameter.next_p->comm+14) == 'h'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
					...
					case 'e':
						...
						if(
							...
							*(ds_parameter.next_p->comm+11) == 'a' &&
							*(ds_parameter.next_p->comm+12) == 'p' &&
							*(ds_parameter.next_p->comm+13) == 'p' &&
							*(ds_parameter.next_p->comm+14) == 's'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
						...
						break;
					case 'f':
#if 0	// fw3a_core. camera related [daemon process]. HRT.
						if(
							*(ds_parameter.next_p->comm+1) == 'w' &&
							*(ds_parameter.next_p->comm+2) == '3' &&
							*(ds_parameter.next_p->comm+3) == 'a' &&
							*(ds_parameter.next_p->comm+4) == '_' &&
							*(ds_parameter.next_p->comm+5) == 'c' &&
							*(ds_parameter.next_p->comm+6) == 'o' &&
							*(ds_parameter.next_p->comm+7) == 'r' &&
							*(ds_parameter.next_p->comm+8) == 'e'
					...
					case 'g':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'l' &&
							*(ds_parameter.next_p->comm+2) == 'g' &&
							*(ds_parameter.next_p->comm+3) == 'p' &&
							*(ds_parameter.next_p->comm+4) == 's'
						)
						{
							//new_type = DS_HRT_TASK;
							new_type = DS_SRT_UI_SERVER_TASK;
					...
					case 'j':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'b' &&
							*(ds_parameter.next_p->comm+2) == 'd' &&
							*(ds_parameter.next_p->comm+3) == '2' &&
							*(ds_parameter.next_p->comm+4) == '/' &&
							*(ds_parameter.next_p->comm+5) == 'm' &&
							*(ds_parameter.next_p->comm+6) == 'm' &&
							*(ds_parameter.next_p->comm+7) == 'c' &&
							*(ds_parameter.next_p->comm+8) == 'b' &&
							*(ds_parameter.next_p->comm+9) == 'l' &&
							*(ds_parameter.next_p->comm+10) == 'k'
						)
						{
							new_type = DS_HRT_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
					...
					case 'k':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 's' &&
							*(ds_parameter.next_p->comm+2) == 'o' &&
							*(ds_parameter.next_p->comm+3) == 'f' &&
							*(ds_parameter.next_p->comm+4) == 't' &&
							*(ds_parameter.next_p->comm+5) == 'i' &&
							*(ds_parameter.next_p->comm+6) == 'r' &&
							*(ds_parameter.next_p->comm+7) == 'q' &&
							*(ds_parameter.next_p->comm+8) == 'd'
						)
						{
							new_type = DS_HRT_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'm' &&
							*(ds_parameter.next_p->comm+2) == 'm' &&
							*(ds_parameter.next_p->comm+3) == 'c' &&
							*(ds_parameter.next_p->comm+4) == 'd'
						)
						{
							new_type = DS_HRT_TASK;
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'e' &&
							*(ds_parameter.next_p->comm+2) == 'y' &&
							*(ds_parameter.next_p->comm+3) == 's' &&
							*(ds_parameter.next_p->comm+4) == 't' &&
							*(ds_parameter.next_p->comm+5) == 'o' &&
							*(ds_parameter.next_p->comm+6) == 'r' &&
					...
					case 'm':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'm' &&
							*(ds_parameter.next_p->comm+2) == 'c' &&
							*(ds_parameter.next_p->comm+3) == 'q' &&
							*(ds_parameter.next_p->comm+4) == 'd'
						)
						{
							new_type = DS_HRT_TASK;
						...
						if(
							...
							*(ds_parameter.next_p->comm+3) == 'i' &&
							*(ds_parameter.next_p->comm+4) == 'a' &&
							*(ds_parameter.next_p->comm+5) == 's' &&
							*(ds_parameter.next_p->comm+6) == 'e' &&
							*(ds_parameter.next_p->comm+7) == 'r' &&
							*(ds_parameter.next_p->comm+8) == 'v' &&
							*(ds_parameter.next_p->comm+9) == 'e' &&
							*(ds_parameter.next_p->comm+10) == 'r'
						)
						{
							new_type = DS_SRT_UI_SERVER_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							*(ds_parameter.next_p->comm+1) == '.' &&
							*(ds_parameter.next_p->comm+2) == 'a' &&
							*(ds_parameter.next_p->comm+3) == 'n' &&
							*(ds_parameter.next_p->comm+4) == 'd' &&
							*(ds_parameter.next_p->comm+5) == 'r' &&
							*(ds_parameter.next_p->comm+6) == 'o' &&
							*(ds_parameter.next_p->comm+7) == 'i' &&
							*(ds_parameter.next_p->comm+8) == 'd' &&
							*(ds_parameter.next_p->comm+9) == '.' &&
							*(ds_parameter.next_p->comm+10) == 'p' &&
							*(ds_parameter.next_p->comm+11) == 'h' &&
							*(ds_parameter.next_p->comm+12) == 'o' &&
							*(ds_parameter.next_p->comm+13) == 'n' &&
							*(ds_parameter.next_p->comm+14) == 'e'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
						...
						if(
							...
							*(ds_parameter.next_p->comm+9) == '.' &&
							*(ds_parameter.next_p->comm+10) == 'm' &&
							*(ds_parameter.next_p->comm+11) == 'u' &&
							*(ds_parameter.next_p->comm+12) == 's' &&
							*(ds_parameter.next_p->comm+13) == 'i' &&
							*(ds_parameter.next_p->comm+14) == 'c'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							...
							*(ds_parameter.next_p->comm+8) == 'd' &&
							*(ds_parameter.next_p->comm+9) == '.' &&
							*(ds_parameter.next_p->comm+10) == 'e' &&
							*(ds_parameter.next_p->comm+11) == 'm' &&
							*(ds_parameter.next_p->comm+12) == 'a' &&
							*(ds_parameter.next_p->comm+13) == 'i' &&
							*(ds_parameter.next_p->comm+14) == 'l'
						)
						{
							new_type = DS_HRT_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							*(ds_parameter.next_p->comm+1) == '.' &&
							*(ds_parameter.next_p->comm+2) == 'b' &&
							*(ds_parameter.next_p->comm+3) == 't' &&
							*(ds_parameter.next_p->comm+4) == '.' &&
							*(ds_parameter.next_p->comm+5) == 'a' &&
							*(ds_parameter.next_p->comm+6) == 'p' &&
							*(ds_parameter.next_p->comm+7) == 'p' &&
							*(ds_parameter.next_p->comm+8) == '.' &&
							*(ds_parameter.next_p->comm+9) == 's' &&
							*(ds_parameter.next_p->comm+10) == 'y' &&
							*(ds_parameter.next_p->comm+11) == 's' &&
							*(ds_parameter.next_p->comm+12) == 't' &&
							*(ds_parameter.next_p->comm+13) == 'e' &&
							*(ds_parameter.next_p->comm+14) == 'm'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						break;
					case 'n':
#if 0	// netd. Network related [daemon process]. HRT.
						if(
							*(ds_parameter.next_p->comm+1) == 'e' &&
							*(ds_parameter.next_p->comm+2) == 't' &&
							*(ds_parameter.next_p->comm+3) == 'd'
						)
						{
							new_type = DS_HRT_TASK;
						...
						if(
							...
							*(ds_parameter.next_p->comm+4) == 'i' &&
							*(ds_parameter.next_p->comm+5) == 'd' &&
							*(ds_parameter.next_p->comm+6) == '.' &&
							*(ds_parameter.next_p->comm+7) == 's' &&
							*(ds_parameter.next_p->comm+8) == 'y' &&
							*(ds_parameter.next_p->comm+9) == 's' &&
							*(ds_parameter.next_p->comm+10) == 't' &&
							*(ds_parameter.next_p->comm+11) == 'e' &&
							*(ds_parameter.next_p->comm+12) == 'm' &&
							*(ds_parameter.next_p->comm+13) == 'u' &&
							*(ds_parameter.next_p->comm+14) == 'i'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
						...
						if(
							...
							*(ds_parameter.next_p->comm+7) == 's' &&
							*(ds_parameter.next_p->comm+8) == 'e' &&
							*(ds_parameter.next_p->comm+9) == 't' &&
							*(ds_parameter.next_p->comm+10) == 't' &&
							*(ds_parameter.next_p->comm+11) == 'i' &&
							*(ds_parameter.next_p->comm+12) == 'n' &&
							*(ds_parameter.next_p->comm+13) == 'g' &&
							*(ds_parameter.next_p->comm+14) == 's'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
					...
					case 'p':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'v' &&
							*(ds_parameter.next_p->comm+2) == 'r' &&
							*(ds_parameter.next_p->comm+3) == '_'
						)
						{
							new_type = DS_SRT_UI_SERVER_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
					...
					case 's':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'e' &&
							*(ds_parameter.next_p->comm+2) == 'r' &&
							*(ds_parameter.next_p->comm+3) == 'v' &&
							*(ds_parameter.next_p->comm+4) == 'i' &&
							*(ds_parameter.next_p->comm+5) == 'c' &&
							*(ds_parameter.next_p->comm+6) == 'e' &&
							*(ds_parameter.next_p->comm+7) == 'm' &&
							*(ds_parameter.next_p->comm+8) == 'a' &&
							*(ds_parameter.next_p->comm+9) == 'n' &&
							*(ds_parameter.next_p->comm+10) == 'a' &&
							*(ds_parameter.next_p->comm+11) == 'g' &&
							*(ds_parameter.next_p->comm+12) == 'e' &&
							*(ds_parameter.next_p->comm+13) == 'r'
						)
						{
...
#endif
#if 0	// system_server. Android system service [application]. X.
						if(
							*(ds_parameter.next_p->comm+1) == 'y' &&
							*(ds_parameter.next_p->comm+2) == 's' &&
							*(ds_parameter.next_p->comm+3) == 't' &&
							*(ds_parameter.next_p->comm+4) == 'e' &&
							*(ds_parameter.next_p->comm+5) == 'm' &&
							*(ds_parameter.next_p->comm+6) == '_' &&
							*(ds_parameter.next_p->comm+7) == 's' &&
							*(ds_parameter.next_p->comm+8) == 'e' &&
							*(ds_parameter.next_p->comm+9) == 'r' &&
							*(ds_parameter.next_p->comm+10) == 'v' &&
							*(ds_parameter.next_p->comm+11) == 'e' &&
							*(ds_parameter.next_p->comm+12) == 'r'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
					...
					case 'u':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'e' &&
							*(ds_parameter.next_p->comm+2) == 'v' &&
							*(ds_parameter.next_p->comm+3) == 'e' &&
							*(ds_parameter.next_p->comm+4) == 'n' &&
							*(ds_parameter.next_p->comm+5) == 't' &&
							*(ds_parameter.next_p->comm+6) == 'd'
						)
						{
						...
						break;
					case 'v':
#if 0	// vold. Volume server [daemon process]. HRT.
						if(
							*(ds_parameter.next_p->comm+1) == 'o' &&
							*(ds_parameter.next_p->comm+2) == 'l' &&
							*(ds_parameter.next_p->comm+3) == 'd'
					...
					case 'w':
						...
						if(
							...
							*(ds_parameter.next_p->comm+3) == '_' &&
							*(ds_parameter.next_p->comm+4) == 's' &&
							*(ds_parameter.next_p->comm+5) == 'u' &&
							*(ds_parameter.next_p->comm+6) == 'p' &&
							*(ds_parameter.next_p->comm+7) == 'p' &&
							*(ds_parameter.next_p->comm+8) == 'l' &&
							*(ds_parameter.next_p->comm+9) == 'i' &&
							*(ds_parameter.next_p->comm+10) == 'c' &&
							*(ds_parameter.next_p->comm+11) == 'a' &&
							*(ds_parameter.next_p->comm+12) == 'n' &&
							*(ds_parameter.next_p->comm+13) == 't'
						)
					...
					case 'I':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'n' &&
							*(ds_parameter.next_p->comm+2) == 'p' &&
							*(ds_parameter.next_p->comm+3) == 'u' &&
							*(ds_parameter.next_p->comm+4) == 't' &&
							*(ds_parameter.next_p->comm+5) == 'D' &&
							*(ds_parameter.next_p->comm+6) == 'e' &&
							*(ds_parameter.next_p->comm+7) == 'v' &&
							*(ds_parameter.next_p->comm+8) == 'i' &&
							*(ds_parameter.next_p->comm+9) == 'c' &&
							*(ds_parameter.next_p->comm+10) == 'e' &&
							*(ds_parameter.next_p->comm+11) == 'R' &&
							*(ds_parameter.next_p->comm+12) == 'e' &&
							*(ds_parameter.next_p->comm+13) == 'a' &&
							*(ds_parameter.next_p->comm+14) == 'd'
						)
						{
							new_type = DS_SRT_UI_SERVER_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
					...
					case 'M':
						...
						if(
							*(ds_parameter.next_p->comm+1) == 'i' &&
							*(ds_parameter.next_p->comm+2) == 'x' &&
							*(ds_parameter.next_p->comm+3) == 'e' &&
							*(ds_parameter.next_p->comm+4) == 'r' &&
							*(ds_parameter.next_p->comm+5) == ' ' &&
							*(ds_parameter.next_p->comm+6) == 'T' &&
							*(ds_parameter.next_p->comm+7) == 'h' &&
							*(ds_parameter.next_p->comm+8) == 'r' &&
							*(ds_parameter.next_p->comm+9) == 'e' &&
							*(ds_parameter.next_p->comm+10) == 'a' &&
							*(ds_parameter.next_p->comm+11) == 'd'
						)
						{
							new_type = DS_SRT_UI_SERVER_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
					...
					case 'R':
						break;
					case 'S':
#if 0	// SurfaceFlinger. [deamon process]. X 4 camera.
						if(
							*(ds_parameter.next_p->comm+1) == 'u' &&
							*(ds_parameter.next_p->comm+2) == 'r' &&
							*(ds_parameter.next_p->comm+3) == 'f' &&
					...
					case 'V':
						break;
					case 'W':
#if 0	// OFF. WindowManager. [daemon process].
						if(
							*(ds_parameter.next_p->comm+1) == 'i' &&
							*(ds_parameter.next_p->comm+2) == 'n' &&
							*(ds_parameter.next_p->comm+3) == 'd' &&
							*(ds_parameter.next_p->comm+4) == 'o' &&
							*(ds_parameter.next_p->comm+5) == 'w' &&
							*(ds_parameter.next_p->comm+6) == 'M' &&
							*(ds_parameter.next_p->comm+7) == 'a' &&
							*(ds_parameter.next_p->comm+8) == 'n' &&
							*(ds_parameter.next_p->comm+9) == 'a' &&
							*(ds_parameter.next_p->comm+10) == 'g' &&
							*(ds_parameter.next_p->comm+11) == 'e' &&
							*(ds_parameter.next_p->comm+12) == 'r'
						)
					...
					case '.':
						...
						if(
							...
							*(ds_parameter.next_p->comm+9) == 'r' &&
							*(ds_parameter.next_p->comm+10) == 'a' &&
							*(ds_parameter.next_p->comm+11) == 't' &&
							*(ds_parameter.next_p->comm+12) == 'e' &&
							*(ds_parameter.next_p->comm+13) == 's' &&
							*(ds_parameter.next_p->comm+14) == 't'
						)
						{
							new_type = DS_SRT_DAEMON_TASK;
							ds_status.ipc_timeout_sec[ds_parameter.next_p->pid] = 0;
	...
	if(ds_status.flag_run_dvs == 1){
		...
		if(ds_status.flag_update_cpu_op == 1){
			...
			if(ds_status.cpu_op_last_update_sec == 0 && ds_status.cpu_op_last_update_usec == 0){
				...
				/* If DS_MIN_CPU_OP_UPDATE_INTERVAL is over since the last update */
				if(ds_counter.elapsed_sec > ds_status.cpu_op_last_update_sec + 1){
					...
					if(ds_counter.elapsed_usec > ds_status.cpu_op_last_update_usec){
						...
						> ds_status.cpu_op_last_update_usec + DS_MIN_CPU_OP_UPDATE_INTERVAL){
						goto update_cpu_op;
					}
					else
						goto do_not_update;
