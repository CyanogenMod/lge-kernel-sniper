...
//	if (lcd_off_boot) return 0;
	if ((err = i2c_transfer(client->adapter, msg, 2)) < 0) 
	{
		...
		dev_err(&client->dev, "[%s-%d] i2c read error [ err = %d ]\n", __func__, __LINE__, err);
		return -EIO;
	}
...
//	if (lcd_off_boot) return 0;
	if ((err = i2c_transfer(client->adapter, &msg, 1)) < 0) 
	{
		...
		dev_err(&client->dev, "[%s-%d] i2c write error [ err = %d ]\n", __func__, __LINE__, err);
		return -EIO;
	}
...
#else
static int kxtf9_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
#endif
...
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2012.03.30]- .ioctl replace to .unlocked_ioctl
 ...
 static int __init kxtf9_probe(struct i2c_client *client, const struct i2c_device_id *id)
 {
	...
	printk("## SHYUN ## kxtf9_probe NON-SKIP Ver.[IN]\n");
