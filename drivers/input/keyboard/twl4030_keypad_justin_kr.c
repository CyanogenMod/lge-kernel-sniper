...
//#include <linux/dvs_suite.h>	//SHYUN_TBD
/* 20110331 sookyoung.kim@lge.com LG-DVFS [END_LGE] */

//20110309 ws.yang@lge.com .. to hub_cp_int_handler
...
#define KEYCODE_HOME				MATRIX_SCAN_CODE(0, 0, TWL4030_ROW_SHIFT)
#define KEYCODE_VOLUMEUP			MATRIX_SCAN_CODE(1, 0, TWL4030_ROW_SHIFT)
#define KEYCODE_VOLUMEDOWN			MATRIX_SCAN_CODE(2, 0, TWL4030_ROW_SHIFT)
//#define KEYCODE_TESTMODE_UNLOCK	MATRIX_SCAN_CODE(0, 0, TWL4030_ROW_SHIFT)
static void twl4030_kp_scan(struct twl4030_keypad *kp, bool release_all)
{
	struct input_dev *input = kp->input;
	u16 new_state[TWL4030_MAX_ROWS];
	int col, row;
	u8 str_text[10] = {0,};	//Press, Release string buffer
	...
	for (row = 0; row < kp->n_rows; row++) {
		...
		for (col = 0; col < kp->n_cols; col++) {
			...
			strcpy(str_text, (new_state[row] & (1 << col)) ? "Press" : "Release");
			...
			if(code == KEYCODE_HOME)
				printk("KEY_HOME %s!\n", str_text);
			else if(code == KEYCODE_VOLUMEUP)
				printk("KEY_VOLUMEUP %s!\n", str_text);
			else if(code == KEYCODE_VOLUMEDOWN)
				printk("KEY_VOLUMEDOWN %s!\n", str_text);
...
unsigned long after10sec = 0;
static irqreturn_t hub_cp_int_handler(int irq, void *_kp)
{

	struct twl4030_keypad *kp = _kp;

#if 0 // ws.yang@lge.com .. for ril recovery
	...
	if(after10sec == 0)
	{
		after10sec = jiffies + 10*HZ;
	}
	else if(!time_after(jiffies , after10sec))
	{
		printk(KERN_ERR"  ******hub_cp_int_handler( return when time over) ******\n");	          
		after10sec = jiffies + 10*HZ;          				
		return IRQ_HANDLED; 
	}      
	after10sec = jiffies + 10*HZ;          		  
#endif		
	{
		printk(KERN_ERR"  ******hub_cp_int_handler( KEY_PROG3 = %d) ******\n",KEY_PROG3);	
		input_report_key(kp->input, KEY_PROG3, 1);
// In ISR, does not use sync object like mdelay()	usnoh@ubiquix.com
//		mdelay(10);
...
// 20100727 jh.koo@lge.com for CP reset Pop up [START_LGE]
	...
	__set_bit(KEY_PROG3, input->keybit);//ws.yang@lge.com.. to key push	
    ret = request_irq(gpio_to_irq(CP_DOWN_INT), hub_cp_int_handler, IRQF_TRIGGER_RISING, "CP_INT", kp);
	if (ret < 0){
