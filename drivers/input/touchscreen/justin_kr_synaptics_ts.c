...
{
	struct irq_desc *desc = irq_to_desc(hub_ts_client->irq);
...
static void synaptics_ts_init_delayed_work(struct work_struct *work)
{
	...
	printk("[TS Fixed] %s : %d / irq_depth = %d\n",__func__,__LINE__, irq_depth);
#else
	printk("[TS NOT Fixed] %s : %d / irq_depth = %d\n",__func__,__LINE__, irq_depth);
#endif
...
do{
	...
	for(ts_cnt = 0; ts_cnt < SYNAPTICS_FINGER_MAX; ts_cnt++)
	{
		...
		if(curr_ts_data.touch_status[ts_cnt])
		{
			if(finger_count == 1 && !ts_cnt)
			{
				...
				input_report_key(ts->input_dev, BTN_TOUCH, true);
				input_mt_sync(ts->input_dev);

				if(!prev_ts_data.touch_status[ts_cnt]) {
					printk("[TS-1] [PRESS] [%d] %d:%d\n",ts_cnt, curr_ts_data.X_position[ts_cnt], curr_ts_data.Y_position[ts_cnt]);
					ts_flag = true;
				}
			}
			else // multi-finger
		...
		prev_ts_data.touch_status[ts_cnt] = curr_ts_data.touch_status[ts_cnt];
	}

	if(!finger_count && (ts_flag == true)) {
		printk("[TS] [RELEASE]\n");
		ts_flag = false;
	}
	...
	{
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2012.03.16] - Keytouch led feature is enabled
#if 0
		if(!key_led_flag_twl && backlight_keyled_flag)		keyled_touch_on();
#endif
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2012.03.16]- Keytouch led feature is enabled
	}
...
static ssize_t hub_ts_FW_show(struct device *dev,  struct device_attribute *attr,  char *buf)
{
	...
	printk("[0] SYSFS_TOUCH_DISABLE_IRQ\n");
	printk("[1] SYSFS_TOUCH_ENABLE_IRQ\n");
	printk("[2] 3SYSFS_TOUCH_GET_IRQ_DEPTH\n");
	printk("========================================================\n");
...
enum {
	SYSFS_TOUCH_DISABLE_IRQ = 0,			// 0
	SYSFS_TOUCH_ENABLE_IRQ,					// 1
	SYSFS_TOUCH_GET_IRQ_DEPTH,				// 2
};
...
{
//  	if(lcd_off_boot == 1)	return;	//SHYUN_TBD
