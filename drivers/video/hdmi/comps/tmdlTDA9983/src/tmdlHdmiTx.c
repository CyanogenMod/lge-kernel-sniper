...
/* Convert DTD to CEA */
static tmdlHdmiTxVidFmt_t dlHdmiTxConvertDTDtoCEA
(
    tmdlHdmiTxEdidVideoTimings_t    *pDTDescriptors
);
...
/* Device library configuration structure completed by tmdlHdmiTxCfgGetConfig with
...
tmdlHdmiTxAudioInfo_t audioInfoListTx = {
    False,
    {TMDL_HDMITX_AFMT_SPDIF, TMDL_HDMITX_AFS_48K,  
     TMDL_HDMITX_I2SFOR_PHILIPS_L,TMDL_HDMITX_I2SQ_16BITS,TMDL_HDMITX_DSTRATE_SINGLE,0x00} 
};
...
/* EDID status */
UInt8 edidDataStatus = HDMITX_EDID_NOT_READ;

/* Flag set in case HDP ACTIVE is detected during initialization */
Bool eventHPDActiveFlag = False;
Bool eventCallHPDActiveCBack    = False;
Bool eventCallRXSensActiveCBack = False;
...
static UInt8 gKsvSecureTimerCounter = 0;
...
)
{
    ...
    tmbslHdmiTxHwFeature_t  bslDeviceCapabilities;
	...
	RETIF( (errCode = tmbslHdmiTxHwGetCapabilities(unit, &bslDeviceCapabilities) ) != TM_OK, errCode)

	switch (bslDeviceCapabilities)
	{
		case tmbslHdmiTxHwNone:    /* None           feature */
			pCapabilities->hdcp = False;
			pCapabilities->scaler = False;
			pCapabilities->audioPacket.HBR = False;
			pCapabilities->audioPacket.oneBitAudio = True;
			pCapabilities->audioPacket.DST = False;
			pCapabilities->hdmiVersion = TMDL_HDMITX_HDMI_VERSION_1_2a;
			pCapabilities->colorDepth = TMDL_HDMITX_COLORDEPTH_24; 
			break;

		case tmbslHdmiTxHwHDCP:    /* HDCP           feature */
			pCapabilities->hdcp = True;
			pCapabilities->scaler = False;
			pCapabilities->audioPacket.HBR = False;
			pCapabilities->audioPacket.oneBitAudio = True;
			pCapabilities->audioPacket.DST = False;
		...
		case tmbslHdmiTxHwScaler:  /* Scaler         feature */
			pCapabilities->hdcp = False;
			pCapabilities->scaler = True;
			pCapabilities->audioPacket.HBR = False;
			pCapabilities->audioPacket.oneBitAudio = True;
			pCapabilities->audioPacket.DST = False;
			pCapabilities->hdmiVersion = TMDL_HDMITX_HDMI_VERSION_1_2a;
		...
		case tmbslHdmiTxHwHDCPScaler:   /* HDCP & Scaler  feature  */
			pCapabilities->hdcp = True;
			pCapabilities->scaler = True;
			pCapabilities->audioPacket.HBR = False;
			pCapabilities->audioPacket.oneBitAudio = True;
...
)
{
    ...
    UInt16                  i;
    UInt8                   deviceVersion;
    tmbslHdmiTxHwFeature_t  featureSupported;
    ...
    /* Take the sempahore */
    RETIF( (errCode = tmdlHdmiTxIWSemaphoreP(dlHdmiTxItSemaphore[unit]) ) != TM_OK, errCode)
    ...
    RETIF_SEM(dlHdmiTxItSemaphore[unit],
        tmdlHdmiTxCfgGetConfig(unit, &gtmdlHdmiTxDriverConfigTable[unit])!= TM_OK, TMDL_ERR_DLHDMITX_INIT_FAILED)
...
)
{
	...
	if (eventHPDActiveFlag == True)
	{
		/* if the HPD active was rased during the initialisation, raise the event HPD */
		dlHdmiTxHandleHPD (instance);
		eventHPDActiveFlag = False;
	}
...
)
{
    ...
    switch (event)
    {
        case TMDL_HDMITX_HPD_ACTIVE: 
            if (eventCallHPDActiveCBack)
            {
                eventCallHPDActiveCBack = False;
                unitTableTx[instance].pCallback(TMDL_HDMITX_HPD_ACTIVE);
            }
        break;

        case TMDL_HDMITX_RX_DEVICE_ACTIVE: 
            if (eventCallRXSensActiveCBack)
            {
                eventCallRXSensActiveCBack = False;
...
)
{
    ...
    for (i = 0; i < gtmdlHdmiTxDriverConfigTable[instance].resolutionNb; i++)
    {
...
)
{
    ...
    tmdlHdmiTxCapabilities_t audioCapabilities;
	...
	/* Audio support */
	RETIF_SEM(dlHdmiTxItSemaphore[instance],
        (errCode = tmdlHdmiTxGetCapabilitiesM (instance, &audioCapabilities)) != TM_OK, errCode)

    

    /* Test if audio input format is supported */
    if ( ((audioInputConfig.format == TMDL_HDMITX_AFMT_OBA) && (audioCapabilities.audioPacket.oneBitAudio == False)) ||
	         ((audioInputConfig.format == TMDL_HDMITX_AFMT_DST) && (audioCapabilities.audioPacket.DST == False)) ||
             ((audioInputConfig.format == TMDL_HDMITX_AFMT_HBR) && (audioCapabilities.audioPacket.HBR == False)) )
    {
        /* Release the sempahore */
        RETIF( (errCode = tmdlHdmiTxIWSemaphoreV(dlHdmiTxItSemaphore[instance]) ) != TM_OK, errCode)
    ...
    if (( sinkType == TMDL_HDMITX_SINK_DVI ) &&
        ((videoOutputConfig.format ==  TMDL_HDMITX_VFMT_06_720x480i_60Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_07_720x480i_60Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_10_720x480i_60Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_11_720x480i_60Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_21_720x576i_50Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_22_720x576i_50Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_25_720x576i_50Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_26_720x576i_50Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_08_720x240p_60Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_09_720x240p_60Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_12_720x240p_60Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_13_720x240p_60Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_23_720x288p_50Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_24_720x288p_50Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_27_720x288p_50Hz) ||
         (videoOutputConfig.format ==  TMDL_HDMITX_VFMT_28_720x288p_50Hz)
        )
       )
    ...
    RETIF_SEM(dlHdmiTxItSemaphore[instance],(errCode = tmbslHdmiTxVideoInSetFine(instance,spSync, HDMITX_PIXTOGL_NO_ACTION) ) != TM_OK, errCode)
        ...
        if (syncIn == EXT)
        {
            switch (videoInputConfig.format)
            {
                ...
                case HDMITX_VFMT_20_1920x1080i_50Hz:
                    if ( (videoOutputConfig.format == HDMITX_VFMT_16_1920x1080p_60Hz) 
                        || (videoOutputConfig.format == HDMITX_VFMT_31_1920x1080p_50Hz) )
                    {
...
)
{
    ...
    if (sinkType == TMDL_HDMITX_SINK_EDID)
    {
        /* Change sink type with the currently defined in EDID */
        RETIF_SEM(dlHdmiTxItSemaphore[instance],
            (errCode = tmbslHdmiTxEdidGetSinkType(instance, (tmbslHdmiTxSinkType_t *)&sinkType) ) != TM_OK, errCode)
    }
	...
	aifChannelCountCode = kChanAllocChanNum[audioInputConfig.channelAllocation] - 1;
        ...
        switch(audioInputConfig.format)
        {
            ...
            case TMDL_HDMITX_AFMT_I2S:
                ...
                if(aifChannelCountCode >= 1) /* For multi-channel */
                {
        ...
        errCode = tmbslHdmiTxSetAudioPortConfig(instance, pEnaAudioPortCfg, pGndAudioPortCfg);
        RETIF_SEM(dlHdmiTxItSemaphore[instance], (errCode != TM_OK) && (errCode != TMBSL_ERR_HDMI_NOT_SUPPORTED), errCode)
        ...
        RETIF_SEM(dlHdmiTxItSemaphore[instance],
            (errCode = tmbslHdmiTxAudioInSetConfig(instance, (tmbslHdmiTxaFmt_t)audioInputConfig.format,  
	     audioInputConfig.channelAllocation, HDMITX_CHAN_NO_CHANGE, HDMITX_CLKPOLDSD_NO_CHANGE, HDMITX_SWAPDSD_NO_CHANGE,
             layout, 0x80) ) != TM_OK, errCode) 
        ...
        RETIF_SEM(dlHdmiTxItSemaphore[instance],
            (errCode = tmbslHdmiTxAudioInSetCts(instance, ctsRef, (tmbslHdmiTxafs_t)audioInputConfig.rate, 
			(tmbslHdmiTxVidFmt_t)instanceStatusInfoTx[instance].pVideoInfo->videoOutConfig.format, 
			vOutFreq, HDMITX_CTS_AUTO, uCtsX, HDMITX_CTSK_USE_CTSX, HDMITX_CTSMTS_USE_CTSX) ) != TM_OK, errCode)
        ...
        RETIF_SEM(dlHdmiTxItSemaphore[instance],
            (errCode = tmbslHdmiTxAudioOutSetChanStatus(instance, HDMITX_CSFI_PCM_2CHAN_NO_PRE, 
			HDMITX_CSCOPYRIGHT_PROTECTED, 0x00, (tmbslHdmiTxafs_t)audioInputConfig.rate, 
			HDMITX_CSCLK_LEVEL_II, HDMITX_CSMAX_LENGTH_20, HDMITX_CSWORD_DEFAULT, 
            HDMITX_CSOFREQ_NOT_INDICATED) ) != TM_OK, errCode)

        /* Set reset_fifo to 1 */
        RETIF_SEM(dlHdmiTxItSemaphore[instance],
            (errCode = tmbslHdmiTxAudioOutSetMute(instance, HDMITX_AMUTE_ON) ) != TM_OK, errCode)
        /* Wait for 20 ms */
        RETIF_SEM(dlHdmiTxItSemaphore[instance],
            (errCode = tmdlHdmiTxIWWait(20) ) != TM_OK, errCode)
        /* Set reset_fifo to 0 */
        RETIF_SEM(dlHdmiTxItSemaphore[instance],
            (errCode = tmbslHdmiTxAudioOutSetMute(instance, HDMITX_AMUTE_OFF) ) != TM_OK, errCode)
        ...
        RETIF_SEM(dlHdmiTxItSemaphore[instance],
            (errCode = tmbslHdmiTxPktSetAudioInfoframe(instance, &pktAif, True) ) != TM_OK, errCode)
    }
...
)
{
    ...
    if (enable == True)
    {
        ...
        pktVif.Colour             = pAviIfData->colorIndicator; /* 3rd api_set_avi_infoframe param */
        pktVif.ActiveInfo         = pAviIfData->activeInfoPresent;
        pktVif.BarInfo            = pAviIfData->barInformationDataValid;
        pktVif.ScanInfo           = pAviIfData->scanInformation;
        pktVif.Colorimetry        = pAviIfData->colorimetry;
#ifndef FORMAT_PC
        pktVif.PictureAspectRatio = kVfmtToAspect_TV[pAviIfData->videoFormatIdentificationCode];
#else /* FORMAT_PC */
        if ((pAviIfData->videoFormatIdentificationCode >= TMDL_HDMITX_VFMT_PC_MIN)
            && (pAviIfData->videoFormatIdentificationCode <= TMDL_HDMITX_VFMT_PC_MAX))
        {
            pktVif.PictureAspectRatio = kVfmtToAspect_TV[pAviIfData->videoFormatIdentificationCode - TMDL_HDMITX_VFMT_PC_MIN + 1];
        }
        else 
	        pktVif.PictureAspectRatio = kVfmtToAspect_TV[pAviIfData->videoFormatIdentificationCode];
#endif /* FORMAT_PC */
        pktVif.ActiveFormatRatio  = pAviIfData->activeFormatAspectRatio;
        pktVif.Scaling            = pAviIfData->nonUniformPictureScaling;

#ifdef FORMAT_PC
        if (pAviIfData->videoFormatIdentificationCode >= HDMITX_VFMT_PC_MIN)
        {
            if (pAviIfData->videoFormatIdentificationCode == HDMITX_VFMT_PC_640x480p_60Hz)
            {
        ...
        else
        {
            ...
            vifInfoFrame = (tmbslHdmiTxVidFmt_t)pAviIfData->videoFormatIdentificationCode;
        ...
        if (((pAviIfData->videoFormatIdentificationCode >= HDMITX_VFMT_06_720x480i_60Hz)
                && (pAviIfData->videoFormatIdentificationCode <= HDMITX_VFMT_09_720x240p_60Hz))
            || ((pAviIfData->videoFormatIdentificationCode >= HDMITX_VFMT_21_720x576i_50Hz)
                && (pAviIfData->videoFormatIdentificationCode <= HDMITX_VFMT_24_720x288p_50Hz)))
        {
        ...
        pktVif.EndTopBarLine      = pAviIfData->lineNumberEndTopBar;
        pktVif.StartBottomBarLine = pAviIfData->lineNumberStartBottomBar;
        pktVif.EndLeftBarPixel    = pAviIfData->lineNumberEndLeftBar;
        pktVif.StartRightBarPixel = pAviIfData->lineNumberStartRightBar;
...
)
{
    ...
    if (enable == True)
    {
        ...
        RETIF_SEM(dlHdmiTxItSemaphore[instance],
            ...
            (tmbslHdmiTxPkt_t *)pVsIfData->vsData, 27, pVsIfData->version, enable) ) != TM_OK, errCode)
    }
    else
    {
        RETIF_SEM(dlHdmiTxItSemaphore[instance], 
...
)
{
    ...
    /* Get the EDID status from BSL driver */
        *pEdidStatus = (tmdlHdmiTxEdidStatus_t)edidDataStatus;
    *pEdidBlkCount = 0;

	if (edidDataStatus == TMDL_HDMITX_EDID_READ)
	{
...
)
{
    ...
    if (writtenDTD > 0)
    {
        ...
        pVideoDesc[0].videoFormat = dlHdmiTxConvertDTDtoCEA(&(edidDTDBuffer[dtdCounter]));
        dtdCounter++;
    ...
    for (i = dtdCounter; i< maxVideoFormats ; i++)
    {
        if ((i < (HDMI_TX_SVD_MAX_CNT + dtdCounter)) && (i < ((*pWrittenVideoFormats) + dtdCounter)))
        {
            ...
            if ((edidVFmtsBuffer[i] & 0x80) == 0x80)
            {
        ...
        else
        {
            if ((dtdCounter < NUMBER_DTD_STORED) && (dtdCounter < writtenDTD))
            {
                ...
                pVideoDesc[i].videoFormat = dlHdmiTxConvertDTDtoCEA(&(edidDTDBuffer[dtdCounter]));
                dtdCounter++;
...
(
    tmInstance_t                    instance,
    tmdlHdmiTxEdidVideoTimings_t    *pPreferredVideoFormat
)
{
    tmErrorCode_t   errCode;
    ...
    RETIF(pPreferredVideoFormat == Null, TMDL_ERR_DLHDMITX_INCONSISTENT_PARAMS)
    ...
    RETIF_SEM(dlHdmiTxItSemaphore[instance], 
        ...
        (tmbslHdmiTxEdidDtd_t *)pPreferredVideoFormat) ) != TM_OK, errCode)
...
)
{
    ...
    /* Get detailled descriptors from EDID, return TMDL_ERR_DLHDMITX_NO_RESOURCES if EDID are not read */
    RETIF_SEM(dlHdmiTxItSemaphore[instance], 
        (errCode = tmbslHdmiTxEdidGetDetailedTimingDescriptors(instance, 
        (tmbslHdmiTxEdidDtd_t *)pDTDescriptors, maxDTDesc, pWrittenDTDesc) ) != TM_OK, errCode)
...
******************************************************************************/
tmErrorCode_t tmdlHdmiTxGetMonitorDescriptors
(
    tmInstance_t                instance,
...
)
{
	...
	gNoWarning = (UInt8)instance;
    if (pEdidFirstMD) gNoWarning = 0;
    if (pEdidSecondMD) gNoWarning = 0;
    if (pEdidOtherMD) gNoWarning = 0;
    gNoWarning = (UInt8)maxOtherMD;
    if (pWrittenOtherMD) gNoWarning = 0;
...
)
{
    ...
    RETIF_SEM(dlHdmiTxItSemaphore[instance], 
        (errCode = tmbslHdmiTxRxSenseGetStatus(instance,
                &rxSenseStatus) ) != TM_OK, errCode)
#else
	rxSenseStatus = HDMITX_RX_SENSE_ACTIVE;
#endif
...
)
{
    ...
    RETIF_SEM(dlHdmiTxItSemaphore[instance],
        (errCode = tmbslHdmiTxHwGetCapabilities(instance, &featureSupported) ) != TM_OK, errCode)
...
)
{
	...
	gNoWarning = (UInt8)instance;
	gNoWarning = (UInt8)enable;
	if (pGamutData) gNoWarning = 0;
...

{
    ...
    unitTableTx[instance].revocationList.pList  = listPtr;
...
)
{
    ...
    {
        pktVif.PictureAspectRatio = kVfmtToAspect_TV[voutFmt - TMDL_HDMITX_VFMT_PC_MIN + 1];
    }
    else 
	    pktVif.PictureAspectRatio = kVfmtToAspect_TV[voutFmt];
#endif /* FORMAT_PC */
    ...
    if (voutFmt >= TMDL_HDMITX_VFMT_PC_MIN)
    {
        ...
        {
            vifInfoFrame = (tmbslHdmiTxVidFmt_t)TMDL_HDMITX_VFMT_01_640x480p_60Hz;
        }
        else
        {
            /* Format PC not Valid in EIA861b */
            vifInfoFrame = (tmbslHdmiTxVidFmt_t)TMDL_HDMITX_VFMT_NULL; 
        }
    }
    else
    {
        ...
        vifInfoFrame = (tmbslHdmiTxVidFmt_t)voutFmt;
        ...
        || ((voutFmt >= TMDL_HDMITX_VFMT_21_720x576i_50Hz) && (voutFmt <= TMDL_HDMITX_VFMT_24_720x288p_50Hz)))
    {
        ...
        pktVif.PixelRepeat = 1; 
    }
    else
    {
        /* Default to no repeat for all other formats */
        pktVif.PixelRepeat = HDMITX_PIXREP_NONE; 
    }
    ...
    switch (voutFmt)
    {
        ...
        case TMDL_HDMITX_VFMT_31_1920x1080p_50Hz:
            pktVif.Colorimetry = (UInt8)TMDL_HDMITX_COLORIMETRY_ITU709;    
            break;

	    default:
            pktVif.Colorimetry = (UInt8)TMDL_HDMITX_COLORIMETRY_ITU601;    
            break;
    }
...
#else
    instance; /* Remove compiler warning */
#endif /* NO_HDCP */
}
...
)
{
   ...
   /* Get the current state */
    ...
    if (dlHdmiTxGetState (instance) == STATE_NOT_INITIALIZED)
	{
		/* Get Hot Plug status */
		errCode = tmbslHdmiTxHotPlugGetStatus(instance, &hpdStatus);

	    if (errCode != TM_OK) return;

		if (hpdStatus == HDMITX_HOTPLUG_ACTIVE)
		{
			eventHPDActiveFlag = True;
		}
		else
		{
			eventHPDActiveFlag = False;
	...
	else
	{
	    ...
	    edidDataStatus = HDMITX_EDID_NOT_READ;
		...
		if (hpdStatus == HDMITX_HOTPLUG_ACTIVE)
		{
			...
			if (powerState == tmPowerOn)
			{
				...
				errCode = tmbslHdmiTxEdidGetBlockData(instance,
					unitTableTx[instance].pEdidBuffer, (UInt32)((unitTableTx[instance].edidBufferSize) >> 7),
					unitTableTx[instance].edidBufferSize, &edidDataStatus);
...
)
{
    ...
    else
    {
        ...
        tmbslHdmiTxSysTimerWait(instance,20); /* ms */
...
/*    if (hpdStatus == HDMITX_HOTPLUG_ACTIVE) 
    {*/
        if (rxSenseStatus == HDMITX_RX_SENSE_ACTIVE)
        {
            ...
            {
                eventCallRXSensActiveCBack = True;
            }
        }
...
static void CommandTaskUnit0()
{
    ...
    while(loop)
    {
        ...
        {
            tmbslHdmiTxHwHandleInterrupt(0);
...
static void CommandTaskUnit1()
{
    ...
    while(loop)
    {
        tmdlHdmiTxIWQueueReceive(unitTableTx[1].queueHandle, &command);

        /* Take the sempahore */
        (void)tmdlHdmiTxIWSemaphoreP(dlHdmiTxItSemaphore[1]);

        /* Clear T0 flag before polling for interrupts */
        instanceStatusInfoTx[1].pColBarState->hdcpSecureOrT0 = False;

        tmbslHdmiTxHwHandleInterrupt(1);

        /* Enable interrupts for Tx (interrupts are disabled in the HandleInterrupt function) */
        tmdlHdmiTxIWEnableInterrupts(TMDL_HDMI_IW_TX_2);

        /* Release the sempahore */
        (void)tmdlHdmiTxIWSemaphoreV(dlHdmiTxItSemaphore[1]);
    };
}
...
static void HdcpTaskUnit0()
{
	...
	tmbslHdmiTxHwGetCapabilities(0, &featureSupported);
    ...
    while(loop)
    {
        ...
        if (gI2CDebugAccessesEnabled == True)
        {
            ...
            if (gKsvSecureTimerCounter) {
                gKsvSecureTimerCounter++;

                if (gKsvSecureTimerCounter == 5) {
#ifdef TMFL_TDA9981_SUPPORT
                    dlHdmiTxHandleR0(0);
#endif
                    gKsvSecureTimerCounter = 0;
                }
...
******************************************************************************/
static void HdcpTaskUnit1()
{
    Bool loop = True;    /* Just to avoid compiler warning */
    tmbslHdmiTxHwFeature_t featureSupported;


	tmbslHdmiTxHwGetCapabilities(1, &featureSupported);
    ...
    while(loop)
    {
        ...
        dlHdmiTxCheckColorBar(1);
        dlHdmiTxCheckHdcpColorBar(1);

#ifndef NO_HDCP
        if ((featureSupported == tmbslHdmiTxHwHDCP) || (featureSupported == tmbslHdmiTxHwHDCPScaler))
        {
            tmbslHdmiTxHdcpCheck(1,35, (tmbslHdmiTxHdcpCheck_t *)&(hdcpInfoListTx[1].hdcpCheckState));
        }
#endif /* NO_HDCP */
...
)
{
    ...
    /* Get the HPD status from BSL driver */
        errCode = tmbslHdmiTxHotPlugGetStatus(instance,(tmbslHdmiTxHotPlug_t *)pHPDStatus);
