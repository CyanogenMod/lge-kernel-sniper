...
extern unsigned int get_wm9093_mode(void);		
#endif
...
#if defined(CONFIG_MACH_LGE_HEAVEN_EVB2) || defined(CONFIG_MACH_LGE_HEAVEN_REV_A)
...
//		schedule_delayed_work(&headset_sw_data->delayed_work,	msecs_to_jiffies(type_detection_tim*3));
	...
	if(hook_status == HOOK_RELEASED){
		...
		if(gpio_get_value(headset_sw_data->hook_gpio) == 0){ 	//threshold area for hook key glitch
		    ...
		    hook_status = HOOK_PRESSED; 
//		    input_report_key(headset_sw_data->ip_dev, KEY_HOOK, 1);
			schedule_delayed_work(&headset_sw_data->hook_delayed_work,	msecs_to_jiffies(10 /*type_detection_tim/8*/)); 
		}
	}
	else{
		if(gpio_get_value(headset_sw_data->hook_gpio) == 0){ 
		    ...
		    input_report_key(headset_sw_data->ip_dev, KEY_HOOK, 1);		    
			input_sync(headset_sw_data->ip_dev);		    
			schedule_delayed_work(&headset_sw_data->hook_delayed_work,	msecs_to_jiffies(hook_detection_tim));		
		}
		else {
			    input_report_key(headset_sw_data->ip_dev, KEY_HOOK, 0);	
				input_sync(headset_sw_data->ip_dev);
...
static irqreturn_t headset_hook_int_handler(int irq, void *dev_id)
{
    ...
    schedule_delayed_work(&switch_data->hook_delayed_work,	msecs_to_jiffies(hook_detection_tim*3)); /* 20110215 jiwon.seo@lge.com for EarJack & Hook Action */
...
static int headsetdet_probe(struct platform_device *pdev)
{
	...
	omap_mux_init_gpio(switch_data->gpio, OMAP_PIN_INPUT_PULLDOWN | OMAP_PIN_OFF_WAKEUPENABLE);	
...
#if defined(CONFIG_MACH_LGE_HUB_REV_A)	|| defined(CONFIG_MACH_LGE_HUB_REV_B)
	//nothing
#else
	...
	switch_data->ip_dev->name = "Hookkey";	
	...
	//nothing
#else
	switch_data->dmb_ant_irq = gpio_to_irq(switch_data->dmb_ant_gpio);	
#endif	
...
static int headsetdet_remove(struct platform_device *pdev)
{
	...
	//nothing
#else	
	gpio_free(switch_data->dmb_ant_gpio);		
#endif
    switch_dev_unregister(&switch_data->sdev);
