...
/* RX definitions                                                   */
/*------------------------------------------------------------------*/
#define HI_WM_MUX_RX                                20
#define LO_WM_MUX_RX                                10
...
static int mux_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
    ...
    if(ipc_tty == NULL)
    {
    	MUX_DBG(3, "%s: ipc_tty = %p, return ECANCELED", __FUNCTION__, ipc_tty);
    	return -ECANCELED;
    }
